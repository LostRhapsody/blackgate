...
use crate::database::{DatabaseManager, MigrationCli, queries};
...
enum Commands {
    ...
    #[command(name = "generate-env")]
    GenerateEnvExample {
        #[arg(long, help = "Output file path", default_value = ".env.example")]
        output: String,
    },
}
...
pub async fn parse_cli_commands(pool: Arc<&SqlitePool>) {
    ...
    match cli.command {
        Commands::AddRoute {
            ...
            println!(
                "Added route: {} -> {} with {} authentication",
                path,
                upstream,
                auth_type_enum.to_display_string()
            );
        }
        ...
        Commands::Metrics { limit, stats } => {
            if stats {
                ...
                if let Some(row) = stats_query {
                    ...
                    println!(
                        "Success Rate: {:.1}%",
                        (row.get::<i64, _>("success_count") as f64
                            / row.get::<i64, _>("total_requests") as f64)
                            * 100.0
                    );
                    println!(
                        "Average Duration: {:.2}ms",
                        row.get::<Option<f64>, _>("avg_duration_ms").unwrap_or(0.0)
                    );
                    println!(
                        "Min Duration: {}ms",
                        row.get::<Option<i64>, _>("min_duration_ms").unwrap_or(0)
                    );
                    println!(
                        "Max Duration: {}ms",
                        row.get::<Option<i64>, _>("max_duration_ms").unwrap_or(0)
                    );
                    println!(
                        "Total Request Bytes: {}",
                        row.get::<Option<i64>, _>("total_request_bytes")
                            .unwrap_or(0)
                    );
                    println!(
                        "Total Response Bytes: {}",
                        row.get::<Option<i64>, _>("total_response_bytes")
                            .unwrap_or(0)
                    );
        ...
        Commands::Start => {
            crate::server::start_server((**pool).clone(), config).await;
        }
        Commands::StartOAuthTestServer { port } => {
            crate::server::start_oauth_test_server((**pool).clone(), config, port.unwrap_or(3001))
                .await;
        }
        Commands::Migrate { action } => {
            ...
            match action {
                MigrateAction::Status => match migration_cli.list_migrations().await {
                    Ok(_) => (),
                    Err(e) => error!("Failed to list migrations: {}", e),
                },
                MigrateAction::ApplyAll => match migration_cli.apply_migrations().await {
                    Ok(_) => (),
                ...
                MigrateAction::ViewSchema => match migration_cli.view_schema().await {
                    Ok(_) => (),
            ...
            if detailed {
                ...
                if !rows.is_empty() {
                    ...
                    for row in rows {
                        ...
                        let status_display =
                            last_health_status.unwrap_or_else(|| "Unknown".to_string());
                        let time_str = response_time
                            .map(|t| format!("{}ms", t))
                            .unwrap_or_else(|| "N/A".to_string());
                        let method_str = method_used.unwrap_or_else(|| "N/A".to_string());
                        let checked_str = checked_at
                            .map(|c| c[..19].to_string())
        ...
        Commands::Collection { action } => {
            match action {
                CollectionAction::List => {
                    ...
                    if !rows.is_empty() {
                        ...
                        for row in rows {
                            ...
                            let default_rate_limit_per_minute: i64 =
                                row.get("default_rate_limit_per_minute");
                            let default_rate_limit_per_hour: i64 =
                                row.get("default_rate_limit_per_hour");
                ...
                CollectionAction::ApplyDefaults { id } => {
                    ...
                    queries::apply_collection_defaults_to_routes(*pool, id)
                        .await
        ...
        Commands::GenerateEnvExample { output } => {
            use std::fs;
            let example_content = crate::env::generate_env_example();
            match fs::write(&output, example_content) {
                Ok(_) => println!("Environment example file generated: {}", output),
                Err(e) => eprintln!("Failed to write environment example file: {}", e),
            }
        }
    }
}
