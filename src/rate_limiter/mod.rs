...
pub const DEFAULT_RATE_LIMIT_PER_MINUTE: u32 = 0; // Default rate limit per minute, none
pub const DEFAULT_RATE_LIMIT_PER_HOUR: u32 = 0; // Default rate limit per hour, none
...
impl RateLimiter {
    ...
    fn is_allowed(&mut self, key: &str, requests_per_minute: u32, requests_per_hour: u32) -> bool {
        ...
        if requests.len() >= requests_per_hour as usize {
            debug!(
                "Rate limit exceeded for key: {} (hourly limit: {})",
        ...
        if minute_requests >= requests_per_minute as usize {
            debug!(
                "Rate limit exceeded for key: {} (minute limit: {})",
...
mod tests {
    ...
    async fn test_rate_limiter() {
        ...
        let mut metrics = RequestMetrics::new("/test".to_string(), "GET".to_string(), 0);

        // First two requests should pass
        assert!(
            check_rate_limit("/test", 2, 100, rate_limiter.clone(), &mut metrics)
                .await
                .is_ok(),
        ...
        assert!(
            check_rate_limit("/test", 2, 100, rate_limiter.clone(), &mut metrics)
                .await
                .is_ok(),
            "2nd request should pass rate limit check"
        );
        ...
        assert!(
            check_rate_limit("/test", 2, 100, rate_limiter.clone(), &mut metrics)
                .await
                .is_ok(),
            "Request should pass rate limit check after waiting"
        );
    }
}
