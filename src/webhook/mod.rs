...
pub fn create_webhook_router() -> Router<AppState> {
    Router::new()
        ...
        .route(
            "/webhooks/health/view",
            axum::routing::get(handlers::handle_health_check_view),
        )
        .route(
            "/webhooks/health/json",
            axum::routing::get(handlers::handle_health_check_json),
        )
        // Error reporting webhook routes
        .route(
            "/webhooks/errors/view",
            axum::routing::get(handlers::handle_error_reporting_view),
        )
        .route(
            "/webhooks/errors/json",
            axum::routing::get(handlers::handle_error_reporting_json),
        )
}
